    ...
    <form class="container p-5" action="{{ route('admin.apartments.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title"
                aria-describedby="TitleHelper" value="{{ old('title') }}" placeholder="p. es. Villa Heraclea" required
                minlength="5">
            <small id="titleHelper" class="text-muted">Inserisci un titolo</small>
        </div>
        <div class="mb-3">
            <label for="rooms">Numero camere*</label>
            <input type="number" min="1" class="form-control @error('rooms') is-invalid @enderror" name="rooms"
                id="rooms" min="1" aria-describedby="roomsHelper" value="{{ old('rooms') }}"
                placeholder="p. es. 3" required pattern="1" max="100" oninput="this.value = Math.abs(this.value)">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="beds" id="beds" min="1"
                aria-describedby="bedsHelper" value="{{ old('beds') }}" placeholder="p. es. 2" required pattern="1"
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('bathrooms') is-invalid @enderror" name="bathrooms"
                id="bathrooms" min="1" aria-describedby="bathroomsHelper" value="{{ old('bathrooms') }}"
                placeholder="p. es. 1" required pattern="1" max="100" oninput="this.value = Math.abs(this.value)">
            <small id="bathroomsHelper" class="text-muted">Inserisci il numero di bagni</small>
        </div>
        <div class="mb-3">
            <label for="square_meters">Metri quadri*</label>
            <input type="number" class="form-control @error('square_meters') is-invalid @enderror" min="1"
                max="30000" name="square_meters" id="square_meters" aria-describedby="square_metersHelper"
                value="{{ old('square_meters') }}" placeholder="p. es. 120" oninput="this.value = Math.abs(this.value)">
            <small id="square_metersHelper" class="text-muted">Inserisci i metri quadri</small>
        </div>
        <div class="mb-3">
            <label for="address">Indirizzo*</label>
            <input type="text" onkeyup="callAddress()" class="form-control @error('address') is-invalid @enderror"
                name="address" id="address" aria-describedby="addressHelper" value="{{ old('address') }}"
                placeholder="p. es. corso Vittorio Emanuele, 335, 93012 Gela CL " required>
        ...
        <input hidden type="number" min="-90" max="90" step="0.000001"
            ...
            aria-describedby="latitudeHelper" value="{{ old('latitude') }}" placeholder="inserisci la latitudine...">
        ...
        <input hidden type="number" min="-180" max="180" step=".000001"
            ...
            aria-describedby="longitudeHelper" value="{{ old('longitude') }}" placeholder="inserisci la longitudine...">
        ...
        <div class="mb-3">
            ...
            <select class="form-select mb-3" id="visible" name="visible" aria-label="Default select"
                style="width: 90px">
        ...
        <div class="mb-3">
            ...
            <select multiple class="form-select" name="services[]" id="services" aria-label="Service" required
                pattern="1">
