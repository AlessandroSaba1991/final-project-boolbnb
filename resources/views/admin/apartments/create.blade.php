...
<form class="container p-5" action="{{route('admin.apartments.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="number" min="1" class="form-control @error('rooms') is-invalid @enderror" name="rooms" id="rooms" min="1" aria-describedby="roomsHelper" value="{{ old('rooms') }}" placeholder="inserisci il numero delle camere...">
        <small id="roomsHelper" class="text-muted">Inserisci il numero di camere</small>
    </div>

    <div class="mb-3">
        <label for="beds">Posti letto</label>
        <input type="number" class="form-control" name="beds" id="beds" min="1" aria-describedby="bedsHelper" value="{{ old('beds') }}" placeholder="inserisci il numero dei posti letto...">
        <small id="bedsHelper" class="text-muted">Inserisci il numero di posti letto</small>
    </div>

    <div class="mb-3">
        <label for="bathrooms">Numero bagni</label>
        <input type="number" class="form-control @error('bathrooms') is-invalid @enderror" name="bathrooms" id="bathrooms" min="1" aria-describedby="bathroomsHelper" value="{{ old('bathrooms') }}" placeholder="numero di bagni...">
        <small id="bathroomsHelper" class="text-muted">Inserisci il numero di bagni</small>
    </div>

    <div class="mb-3">
        <label for="square_meters">Metri quadri</label>
        <input type="number" class="form-control @error('square_meters') is-invalid @enderror" min="1" name="square_meters" id="square_meters" aria-describedby="square_metersHelper" value="{{ old('square_meters') }}" placeholder="inserisci la dimensione dell'immobile...">
