...
<form action="{{route('admin.apartments.update', $apartment->id)}}" method="post" enctype="multipart/form-data">
  ...
  <div class="mb-3">
    ...
    <input type="address" class="form-control @error('address') is-invalid @enderror" name="address" id="address" aria-describedby="addresshelp" value="{{old('address', $apartment->address)}}" formnovalidate oninput="this.value = Math.abs(this.value)">
    <div id="addresshelp" class="form-text">Inserire l'indirizzo</div>
  </div>
  <!-- ./input address  -->
  <div class="mb-3">
    <label for="latitude" class="form-label">Latitudine</label>
    <input type="number" min="-90" max="90" step="0.000001" class="form-control @error('latitude') is-invalid @enderror" name="latitude" id="latitude" aria-describedby="latitudehelp" value="{{old('latitude', $apartment->latitude)}}" formnovalidate oninput="this.value = Math.abs(this.value)">
